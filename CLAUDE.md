...
```
Step 1: 構造分析 → ユーザー確認（★確認ゲート）
  パターン判定: furniture数 / エリア構成 / 建築混在 / WS / 年次・関連案件
  → 確定したパターンを以降の全Stepで常に参照する

Step 2: データ生成
  ナレッジ / 金物 / specs / コスト / ドキュメント / 写真を収集・振り分け
  → 1件ずつStep 1の構造を参照しながら振り分ける

Step 3: クロスチェック → ユーザー確認（★確認ゲート）
  振り分け後のデータをfurniture単位・案件単位で俯瞰検証
  → 混入・矛盾・欠落を検出し、修正後に完了判定+ドキュメント生成
...
**見積・コストの時系列分析ルール（Step 2で実行）:**

見積は案件進行中に何度も更新される。最終値を正しく拾うために時系列で整理する:

1. **コスト関連メッセージ + PDF/Excelを時系列順に並べる**
2. **発信元を分類**:
   - VUILD内部概算（MTGメモ、チャット内の金額言及）
   - 外注先見積（衣川、カトケン、千波等 → PDF受領）
   - クライアント予算（パナソニック側の予算枠）
3. **変遷を追跡**: 初期概算 → 減額検討 → 修正見積 → 最終確定
4. **cost_breakdownには最新/最終値を採用** — 古い値は `cost_history[]` に保持
5. **PDF/Excelの中身を分析**: DL → Read tool で金額・内訳を抽出 → furniture別に振り分け
6. **furniture単体の原価 vs フロア/案件予算を分離**:
   - 「6FL約1000万」→ 案件の cost_overview（フロア予算）
   - 「MKU約50万/台」→ furnitureの cost_breakdown（単体コスト）
...
| `estimate` | 見積書 | 見積タブ |
| `invoice` | 請求書 | 見積タブ |
| `drawing` | 図面（PDF/画像） | **図面タブ** |
| `fabrication` | 加工依頼書・制作指示書 | **図面タブ** |
| `reference` | 参考資料・仕様書・リスト | 案件ドキュメントタブ |
| `contract` | 契約書 | 案件ドキュメントタブ |
| `schedule` | 工程表・スケジュール | 案件ドキュメントタブ |

**図面タブに表示するもの:**
- PDF図面（三面図、施工図、制作図）
- 加工依頼書（CNC加工指示、素材手配指示）
- 画像図面（従来の `project.drawings[]`）
- → `documents[]` に `type: "drawing"` or `"fabrication"` で登録すれば図面タブに表示
...
- PDF見積から確定値を抽出した場合も文字列で格納（例: `"¥2,500,000"`, `"¥648,450"`）
- 手動入力で確定値が得られた場合は数値で格納
- UI側は `typeof v === 'number'` で判定し、数値ならformatPrice、文字列ならそのまま表示
- **カード表示用**: cost_breakdownから¥金額のみを抽出（括弧内の仕様テキストは除去）。カードには仕様を書かずサイズ(dimensions)のみ表記。

**furnitureのサイズ(dimensions)取得ルール:**
- 第1優先: Slackメッセージ内のサイズ表記（W×D×H, mm/cm単位）
- 第2優先: 見積PDF内の寸法記載（例: 900×900×h700）
- 第3優先: 加工依頼書・図面内の寸法
- 第4優先: 3Dモデルのバウンディングボックス
- フォーマット: `"W900×D900×H700"` or `"約4000×1100"` （単位mm、概寸の場合は「約」付与）
- サイズ不明の場合: `dimensions: null`（カードにサイズは表示しない）

**furniture specs（素材・仕上げ・構造）の深掘りルール（Step 2で実行）:**

specsの3フィールド（material / finish / note）を、以下の情報源から詳細に抽出する:

1. **Slackメッセージ分析**（最重要）:
   - 素材キーワード: 合板種類（シナ共芯/構造用/ラワン/MDF）+ 厚み（t:24mm等）
   - 塗装キーワード: 塗料名（ウレタン/キシラデ/シーラー/ステイン）+ 艶（3分艶等）+ 工程（下塗り→仕上げ）
   - 加工キーワード: CNC種類（ShopBot/ビエッセ）+ 接合方法（cabineo/ビス/ダボ/ボルト）
   - 外注先情報: 誰が何を担当（例: 天野さん=ウレタン塗装、衣川=芝貼り、カトケン=CNC加工）
   - 設備情報: 品番・メーカー（例: 冷蔵庫NR-F557XV）

2. **見積PDF分析**: 品目名に素材・仕様が含まれる（例: 「構造用合板 t:24mm 3×6サイズ」）

3. **加工依頼書・図面**: 素材指定・寸法・加工指示

4. **ナレッジメッセージ**: 製作中の気づき・トラブル → note に反映

**specsの記述精度基準:**
- material: 合板の種類 + 厚み + 用途部位を明記（例: 「シナ共芯合板 24mm（本体）+ MDF（凹凸パネル下地）」）
- finish: 部位別の塗装仕様を明記（例: 「天板: 桜葉ステイン / 側面: シーラー+ステイン / 凹凸: 砂飯盒塗装」）
- note: 加工外注先・施工担当・特記事項（例: 「CNC加工: カトケン、塗装: 天野さん3分艶ウレタン」）
- **「合板」「塗装仕上げ」のような曖昧な記述はNG** → 種類・厚み・工法まで特定する
- **エリアマッピングとspecsの整合性チェック**: 仕上げ・塗装の情報をfurnitureに振り分ける際、必ずエリアマップと照合する。同じチャンネル内で複数エリアの塗装情報が飛び交うため、どのエリアの話かを文脈から特定してから振り分ける（例: 桜葉ステイン=7F、ウレタン=6F）

**カードUI統一ルール:**
ポータル(index.html)と案件内(detail-template.html)のfurnitureカードは以下を統一する:
- サムネイル: サイズ別ルールに従う（大型→写真、中小→モデル画像）
- 名前: furniture名
- サイズ: dimensions のみ（仕様・仕上げは書かない）
- コスト: ¥金額 + 見積ベース/実績 の表記
...
| 要素 | 第1優先 | 第2優先 | 第3優先 |
|------|--------|--------|--------|
| furnitureカードのサムネイル | 下記「サイズ別サムネイルルール」参照 | — | — |
| furnitureカードのコスト | `production_cost` | `cost_breakdown`から見積値抽出 | 「コスト未取得」 |
| furnitureヒーロー画像 | アクソメSVG | ViewCapture（3Dパース） | 空欄 |

**furnitureカードのサイズ別サムネイルルール:**

furnitureのサイズに応じてカードに表示するサムネイルの種類を変える。
ポータル(index.html)と案件内(detail-template.html)で **統一する**。

| サイズ分類 | 対象例 | カードサムネイル | フォールバック |
|-----------|--------|----------------|-------------|
| **大型什器**（キッチンサイズ以上: 造作キッチン、屋台、小屋、大型什器） | pan-kitchen-6f, pan-kitchen-7f | `gallery_photos[0]`（設置写真） | プレースホルダー文字 |
| **中小什器**（テーブル以下: ベンチ、スツール、ラック、パーティション等） | onomichi-bench-a, pan-denzai-stool | `hero_photos[0]`（モデル画像/アクソメ） | **生成画像**（AI生成 or レンダリング） → プレースホルダー文字 |

**判定基準:**
- `category` が「キッチン」「造作」「屋台」「小屋」「大型什器」 → 大型什器
- それ以外（「ベンチ」「スツール」「テーブル」「収納」「パーティション」「什器」等） → 中小什器
- furniture.json に `size_class` フィールドを追加して明示的に指定も可（"large" | "small"）

**中小什器のモデル画像がない場合:**
1. アクソメSVG/ViewCapture を探す（スキャン時に自動検出）
2. なければ **生成画像**（AI画像生成 or Rhinoレンダリング）を作成 → `photos/placeholders/gen-{id}.jpg`
3. それもなければプレースホルダー文字（最終手段）
...
| ディテール寄り（金物・接合部・仕上げ・天板アップ・丸脚等） | そのfurnitureページ gallery **のみ** | galleryに含む（furnitureでは重要 — 積極的に採用） |
...
複数年案件: 別案件として分離し、各案件の gallery_photos に振り分ける（年次タブは使わない → 「関連案件の整理ルール」参照）
...
| タイムスタンプ | 設置日・納品日付近 → 完成写真 / 年度判定（複数年案件の振り分け用） | 中 |
...
4. 複数年案件: タイムスタンプで年度を判定 → 該当案件の gallery_photos に振り分け（年次タブは使わない）
...
"ws_events": [
  {
    ...
    "documents": [{"label": "素材サンプル一覧", "url": "", "type": "reference"}]
  }
]
```

**WSの検出と分析（Step 1 構造分析で必ず実行）:**

WSが検出された場合、以下の分析を構造分析レポートに含める:

1. **WS検出**: メッセージ内の「WS」「ワークショップ」「セルフビルド」「塗装体験」「組立体験」等
2. **各WSイベントを特定**: 日付・名前・対象furniture・参加者
3. **WS写真の判別**（以下を区別すること）:
   - ✅ WS写真: 参加者（クライアント社員等）が実際に作業・選定している写真
   - ❌ 製作ナレッジ: 職人（しゅうへいさん等）の製作作業 → WSではなくproduction scope
   - ❌ 竣工後の使用シーン: WSとは無関係（芝キューブで寝そべり等）
   - ❌ ファイル名だけでWS判定しない（例: panasonicws-*.jpgが竣工写真だった実績あり）
4. **WS関連ドキュメント**: 素材サンプル一覧、参加者リスト、スケジュール等を収集
5. **ユーザーに確認**: WSイベント一覧 + 写真候補を提示
6. **WS写真の枚数:** 各WSイベントごとに **max 5枚**
...
| WSの加工・塗装のコツ（しゅうへいさんの作業等） | **製作ナレッジ**（WSドキュメントではない） → scope: "production" |

**案件ページUIの「WS」タブ:**
- WSイベントをカード形式で表示
- 各カード: 名前 + 日付 + 対象furniture + 参加者 + 概要 + 写真ギャラリー + 関連ドキュメント
- furnitureへのリンクバッジ付き
- **教訓セクションは不要**（削除済み）
...
- [ ] 構造分析レポートを作成してユーザーに提示:
  ...
  - チーム体制（チャンネル参加者の投稿内容・役割言及から抽出。**他案件のチームをコピーしない**）
    - **VUILDメンバー** と **外注先** を区別する:
      - VUILDメンバー: team[] に登録（クレジットに表示）
      - 外注先（衣川さん、カトケンさん、千波さん等）: team[] に入れない → ドキュメント/コスト欄で言及
    - 判定基準: 「〜さんに依頼」「〜見積受領」「外注先」「手配」→ 外注先
    - VUILDのSlackワークスペースにいても外注先の場合がある（投稿数だけで判断しない）
    - 不明な場合はユーザーに確認する
    - **名前の取得**: `users.info` APIでUID→real_nameを自動取得する（推測しない）
    - **メンバー選定基準**: 投稿数上位をメインメンバーとして登録（目安: 全投稿の5%以上）
    - **同一ロールの複数人**: 製作・ディレクション等は複数人で担当することがある → 投稿内容から判断して複数人登録OK（例: 制作管理2名、設計2名）
  - **WSイベント分析**（WS検出時は必ず実行 — 5.1「WSの検出と分析」参照）:
    - 各WSの名前・日付・対象furniture・参加者
    - WS写真 vs 製作ナレッジ vs 竣工使用シーン の区別
    - WS関連ドキュメントの収集
...
- [ ] **チェック1: 混入チェック（違う部屋の荷物が入ってないか）**
  各furnitureの部屋の中身を並べて「場違いなもの」を探す:
  - ナレッジ: そのfurnitureに紐づいたナレッジを全部読み、話が通るか確認
    → 別エリア/別furnitureの話が混入していないか（例: 7Fの部屋に6Fの塗装情報）
  - 写真: 集まった写真を並べて、全て同じfurnitureが写っているか確認
    → 別furnitureが主役の写真が紛れていないか
  - 金物: そのfurnitureの構造に対して金物リストが妥当か確認
    → 別furnitureの金物が紛れていないか（例: ベンチの部屋にキッチン水栓金具）
  - コスト: 金額感がspecs/サイズと整合するか確認
    → 案件全体の見積がfurniture単体に紛れていないか
  - 混入を検出した場合 → 正しいfurnitureへの移動を提案

- [ ] **チェック2: 整合性チェック（部屋の中で情報が矛盾してないか）**
  同じfurnitureの情報同士を突き合わせる:
  - specs × ナレッジ: 素材・塗装の記述が一致するか
    → 矛盾がある場合、時系列で後の方が最終仕様の可能性を検討
  - specs × 写真: 写真で見える仕上げ（色・艶・素材感）とspecsが一致するか
    → エリア違いの情報が混入している可能性を検討
  - コスト × ドキュメント: cost_breakdownと見積PDFの金額が一致するか
    → 古い見積の値が残っていないか、時系列を再確認
  - ナレッジのscope: production/designの分類が正しいか再確認
    → 案件に分類されたがproductionの方が適切なもの、その逆がないか
  - 矛盾を検出した場合 → 原因の推定 + ユーザーに確認

- [ ] **チェック3: 欠落チェック（拾い漏れがないか）**
  各furnitureの情報充足度を一覧化する:
  ```
  ...
  furniture C | ❌   | ❌   | ❌      | ❌  | ❌    | ❌      |
  ```
  - 全て❌のfurniture: Phase 1で認識されたのに情報が拾えていない
    → 別furnitureと一緒に語られていて個別振り分けできなかった可能性 → 再確認
  - 写真はあるのにナレッジ/コストがゼロ: 情報が案件レベルに落ちている可能性
    → 案件の部屋にそのfurniture関連の情報が紛れていないか確認
  - 特定カテゴリだけ欠落: 正常な場合もある（シンプルな製品等）
    → 正常/異常の判断をユーザーに提示

- [ ] **チェック4: 案件レベルの俯瞰**
  案件全体のバランスと整合性を確認する:
  - 写真バランス: 特定furnitureに偏っていないか、各エリアから選ばれているか
  - コスト整合: furniture単体コストの合計 vs 案件全体コスト → 大きく乖離していないか
    → 差額が説明できるか（工務店手間・運送費・設備費等）
    → 異常な乖離 → 振り分け漏れの可能性
  - 案件ページの残留物: 案件のdesignナレッジを再読して、productionに移すべきものがないか

- [ ] **チェック結果をユーザーに提示**:
  ```
  ■ 混入（自動修正提案）:
    1. [アイテム]: [現在の場所] → [移動先] に移動
    → これらを移動してOKですか？

  ■ 矛盾（確認が必要）:
    1. [furniture名]: [情報A] vs [情報B] — どちらが最終仕様ですか？

  ■ 欠落（情報不足）:
    1. [furniture名]: [不足カテゴリ] — 情報が少ないのは正常ですか？

  ■ scope修正提案:
    1. [ナレッジ]: [現在のscope] → [提案scope] に変更
    → OKですか？
  ```
  - 問題なし → 完了判定へ
  - 混入の自動修正 → ユーザーOK後に修正して完了判定へ
  - 矛盾・欠落 → ユーザー回答を受けて修正して完了判定へ

- [ ] **完了判定 + ドキュメント生成**（クロスチェック完了後に実行）:
  - 案件のstatus確認
  - completed → 製作ドキュメント + 案件ドキュメントを必ず生成（5.1「完了判定ルール」に従う）
  - 各セクションの充足度チェック → 不足セクションにプレースホルダー設置
  - active → ドキュメント生成はスキップ（ナレッジ蓄積のみ）
  - ※ クロスチェックで修正済みのデータを使うため、ここで生成する
...
**対象チャンネルの選定:**
- `data/scan_state.json` のactiveチャンネルのみ対象
- Slack APIの `oldest` パラメータで `last_scanned_ts` 以降の新着のみ取得
