...
- 案件名（projects.json の `name`）は **Slackチャンネル名** をベースにする
- フォーマット: `#{番号} {チャンネル名}` （例: `#1125 Panasonic Design Kyoto`、`#3035 尾道倶楽部 FFE`）
- 関連案件（WS、年次違い）は親案件名を含める: `#{番号} {メイン案件名} - {サブ名}` （例: `#1125 電材プロダクトWS`）
